...
An AI agent that signs Ethereum transactions using AgentaOS threshold wallet. The full private key never exists.
...
1. The agent loads a threshold signer from the `AGENTA_API_SECRET` env var
...
4. Each transaction is co-signed by the agent's share + the AgentaOS server's share (2-of-3 MPC)
